...
{
<#
Please read the README.MD for this PowerSploitUpdate2021 version of Get-GPPPassword
All credit to goes to original authors of Get-GPPPassword which is found at the following link.
...
#>
    ...
    [decimal] $global:countofPasswordInstances = 0


    # For a production run of this set the value $StringToSearchFor to cPassword.
    # For a semi-proof test showing that the script works correctly, set the value $StringToSearchForinXML to "Group".
    # The semi-proof test simply provides evidence that the script does find all applicable types of files with "Group" in the text.
    # Using "Group" instead gives a little more confidence when you are getting a result of 0 files found for cPassword.
    # It gives a litle more confidence that cPassword 0 files isn't a false negative.
    # [string] $StringToSearchForinXML = 'cPassword'
    # [string] $StringToSearchForinXML = 'Group'
    ...
    [CmdletBinding()]
        ...
        try {
            ...
            if ($Xml.innerxml -match $StringToSearchForinXML) 
            {
                $global:countofPasswordInstances = $global:countofPasswordInstances+1
                

                if ($StringToSearchForinXML = "Group")
                {
                     Write-Verbose "The following file contains 'Group' XML text."
                     Write-Verbose $File
    ...
    # BEGINNING PROCESSING OF Get-GPPPassword  - tja

    # If $VerbosePreference to original = "SilentlyContinue" then Write-Verbose messages will not appear.  - tja
    # Save current $VerbosePreference setting to set it back at end of this script. - toddalbers
    ...
    # $VerbosePreference="Continue" ensures that Write-Verbose prints output. - tja
    ...
    try 
        {
            ...
            Write-Verbose "[Get-GPPPassword] Found $($XMLFiles | Measure-Object | Select-Object -ExpandProperty Count) files that are of the 6 file xml types that could contain passwords."
            ...
            [string] $countofPasswordInstancesString = $global:countofPasswordInstances.ToString()

            Write-Verbose "The number of instances that '$StringToSearchForinXML' was found in these files is $countofPasswordInstancesString"
    ...
    # Return VerbosePreference to original setting - toddalbers
